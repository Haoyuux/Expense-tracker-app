...
// Fix: Initialize GoogleGenAI with the required named parameter and environment variable
...
export const getSpendingInsights = async (transactions: Transaction[]): Promise<SpendingInsight[]> => {
  ...
  try {
    // Fix: Using the recommended model for general reasoning tasks and correct content structure
...
export const parseReceipt = async (base64Image: string): Promise<ReceiptData | null> => {
  try {
    // Fix: Updated contents to use the correct { parts: [...] } structure for multi-part inputs (image + text)
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: {
        parts: [
